...
def separate_two_stems(path: Path, output_dir: Path) -> tuple[Path, Path]:

    #separate vocals from backing track using demucs
    #outputs vocals.mp3 and backing.mp3 directly to output_dir

    subprocess.run(
        ["python", "-m", "demucs", "--two-stems=vocals", "--out", str(output_dir), str(path)],
        check=True,
    )

    stem = output_dir / "htdemucs" / path.stem
